...
int main(int argc, char* argv[]) {
    ...
    for (double target_distance : distances) {
        ...
        RUNTIME_ASSERT(!associations.empty());
        ...
        for (double y = -2.31; y < 2.31; y += 0.01) // 2.31 = max_distance * tan(hfov / 2)
            scene.addPoint(Eigen::Vector3d(std::abs(line(2)) + 0.2062, y, 0.0), MRPTScene::COLOR_BLACK);
...
*/
        ...
        for (unsigned int i = 0; i < height; i += 4) {
            for (unsigned int j = 0; j < width; j += 4) {
                const double z = depth_image.image(i, j);
                if (IsNaN(z) || IsInf(z)) continue;
                if (z < DepthCamera::depthMin() || z > DepthCamera::depthMax()) continue;
                ...
                if (calibration->at(i / 4, j / 4).meaningful) {
                    if (FLAGS_compensate) {
                        ...
                        Eigen::Vector3d point = DepthCamera::T_wf() * (l * z_);
                        point(2) = 0.0;

                        scene.addPoint(point,
                                       MRPTScene::COLOR_BLUE);
                    } else {
                        Eigen::Vector3d point = DepthCamera::T_wf() * (l * z);
                        point(2) = 0.0;
